...
const argv = yargs
    .command('workbook-uuid', 'Print list of workbook uuids in Honeycode.', {
    })
    .command('workbooks', 'Print list of workbook arns in Honeycode.', {
    })
...
const parameterLogin    = argv.username;
    ...
    console.log("-------------\t-------------");
    for (var i=0; i<workbookCount; i++) {
        let workbookArn = foundWorkbooks[i].workbookUUID.split(":");
        console.log(foundWorkbooks[i].workbook + " => " + workbookArn[workbookArn.length -1]);
    }
}
...
if (argv._.includes('workbooks-uuid')) {
    workbookUUIDs();
}
